...
const AuthModal = () => {
  ...
  const checkErrors = () => {
    ...
    if (password.length < 6)
      setLogInErrorMsg(
        "Please enter password of at least 6 characters in length"
      );

    if (email.length < 8 && password.length < 8) {
      setLogInErrorMsg(
        "Please enter valid email address and password to continue"
      );
    }
  };
  ...
  const handleResetPassword = async (e) => {
    ...
    if (email.length >= 6) {
      ...
      try {
        ...
        setForgotPasswordError("Error! Email not found.");
        console.log(err);
      }
    }
  };
  ...
  return (
    <>
      {modalState.open && (
        <div className="max-w-4xl mx-auto flex items-center justify-center overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none">
          {emailSent ? (
            <div className="my-5 mx-auto w-4/5 sm:w-1/2 h-96 bg-white flex flex-col rounded-2xl shadow-lg outline-none">
              <div className="cursor-pointer pt-8 pb-14" onClick={handleClose}>
                <p className="text-right text-2xl text-gray-400 font-bold pr-12 hover:text-amber-500">
                  ✕
                </p>
              </div>
              <div className="text-center flex flex-col justify-center">
                <p className="text-2xl lg:text-3xl font-semibold text-gray-600 tracking-wider pb-8">
                  Email Sent
                </p>
                <p className="text-lg px-4 text-gray-700 tracking-wider leading-relaxed">
                  Please check your email to reset your password 💫 Check spam
                  just in case
                </p>
                <p
                  className="text-red-500 tracking-wide text-center pt-9 cursor-pointer hover:text-gray-700"
                  onClick={turnOff}
                >
                  Return
                </p>
              ...
              {forgotPassword ? (
                <>
                  <div className="flex flex-row items-center justify-between mx-4 px-4 py-10 border-b">
                    <p className="text-2xl lg:text-3xl font-semibold text-gray-500 tracking-wider">
                      Forgot Password
                    </p>
                    <div className="cursor-pointer" onClick={handleClose}>
                      <p className="text-2xl text-gray-400 font-bold pr-4 hover:text-amber-500">
                        ✕
                      </p>
                    </div>
                  </div>
                  <form className="py-6 px-10">
                    ...
                    {forgotPasswordError && (
                      <p className="text-center text-red-500 pb-6">
                        {forgotPasswordError}
                      </p>
                    )}
